...
Add an 'appName' property in your arapp.json with your app's ENS name`
  ...
  if (arapp.appName) {
    ...
    const allNamesAreEqual = appNamesArr.every(
      (name) => name === appNamesArr[0]
    )
 ...
 * @param network
 */
export function getEnsRegistry(arapp: AragonAppJson, network?: string): string {
  if (!network) {
    throw new AragonPluginError('Missing network information')
  }
